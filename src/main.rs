...
use crate::arrow_structs::flush_partitioned;
use crate::pb::sf::solana::raw::blocks::rewards::v1::BlockReward;
use crate::pb::sf::substreams::v1::module::input::{Input, Params};
use crate::pb::sf::solana::raw::blocks::rewards::v1::Output;
use prost::Message;
use std::fs;
use std::path::Path;
use std::sync::Mutex;
...
const CURSOR_FILE: &str = "./cursor.yaml";
const BATCH_SIZE: u64 = 3000;
...
lazy_static! {
    static ref BATCH_STATE: Mutex<BatchState> = Mutex::new(BatchState::new());
}

struct BatchState {
    buffer: Vec<BlockReward>,
    min_slot: u64,
    max_slot: u64,
    blocks_accum: usize,
}
...
async fn main() -> Result<(), Error> {
    ...
    std::fs::create_dir_all("./out").context("failed to create output directory `./out/`")?;
    ...
    loop {
        match stream.next().await {
            ...
            Some(Ok(BlockResponse::New(data))) => {
                ...
                persist_cursor(data.cursor, clock.number)?;
            }
            Some(Ok(BlockResponse::Undo(undo_signal))) => {
                process_block_undo_signal(&undo_signal)?;
                let block = undo_signal.last_valid_block.as_ref().unwrap();
                persist_cursor(undo_signal.last_valid_cursor, block.number)?;
            }
            Some(Err(err)) => {
                println!();
...
fn process_block_scoped_data(data: &BlockScopedData) -> Result<(), Error> {
    // 1) Decode the Substreams Output wrapper:
    let map_out = data.output.as_ref().unwrap().map_output.as_ref().unwrap();
    let out: Output = Output::decode(map_out.value.as_slice())?;

    // 2) Lock our global batch state and update it
    let mut state = BATCH_STATE.lock().unwrap();

    for tx in out.data.into_iter() {
        let slot = tx.block_slot as u64;
        state.min_slot = state.min_slot.min(slot);
        state.max_slot = state.max_slot.max(slot);
        state.buffer.push(tx);
    }

    // 3) Count one more block, flush every 1 000 blocks
    state.blocks_accum += 1;
    if state.blocks_accum >= BATCH_SIZE as usize {
        // take out the data
        let batch = std::mem::take(&mut state.buffer);
        state.blocks_accum = 0;
        state.min_slot = u64::MAX;
        state.max_slot = 0;
        drop(state);

        // spawn async writer (or do inline)
        task::spawn_blocking(move || {
            flush_partitioned(batch).map_err(|e| eprintln!("partitioned flush failed: {:#}", e))
        });
    }
...
fn finalize_batches() -> Result<(), Error> {
    let mut state = BATCH_STATE.lock().unwrap();

    // if we saw any blocks and have a non-empty buffer
    if state.blocks_accum > 0 && !state.buffer.is_empty() {
        // steal the leftover batch out of the state
        let batch = std::mem::take(&mut state.buffer);

        // write *all* remaining rows into partitioned files
        flush_partitioned(batch)?;

        // reset accumulators in case finalize_batches ever runs twice
        state.blocks_accum = 0;
    }
...
fn persist_cursor(cursor: String, block_number: u64) -> Result<(), anyhow::Error> {
...
fn load_persisted_cursor() -> Result<Option<String>, anyhow::Error> {
    ...
    let contents = fs::read_to_string(CURSOR_FILE)?;
    let state: CursorState = serde_yaml::from_str(&contents)?;
